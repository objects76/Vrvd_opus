/* scapdec.cpp - two codec paths, selected at RUNTIME by each packet's magic
 * (scap_packet.h) - no configuration, the decoder follows whatever arrives:
 *
 * zstd (SCAP_MAGIC): streaming decompress + rect blit into an 8bpp
 ...
 *
 * AV1 (SCAP_MAGIC_AV1): each packet is one AV1 temporal unit; Av1Dec (dav1d,
...
struct ScapDec
{
    ...
    bool     canvas32 = false;  /* 32bpp BGRA (av1) vs 8bpp palette (zstd) */
    HDC      memDC = nullptr;
    HBITMAP  dib = nullptr;
...
static int CreateCanvas(ScapDec* d, int w, int h, bool canvas32)
{
    DestroyCanvas(d);
                                     ...
                                     unsigned size, RECT* updatedBounds)
{
    ...
    if (hdr.magic == SCAP_MAGIC_AV1)
    {
        ...
        if (hdr.width != d->width || hdr.height != d->height || !d->canvas32)
            if (CreateCanvas(d, hdr.width, hdr.height, true) != 0)
                return -2;
    ...
    if (hdr.width != d->width || hdr.height != d->height || d->canvas32)
        if (CreateCanvas(d, hdr.width, hdr.height, false) != 0)
            return -2;
