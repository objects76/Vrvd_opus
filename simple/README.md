...
아래 표의 scapenc/scapdec 설명은 zstd 경로 기준이다. 코덱은 접속마다 뷰어가
런타임에 고른다 — "코덱 선택 (런타임, viewer `--codec`)" 절 참조.
...
### 코덱 선택 (런타임, viewer `--codec`)

코덱은 접속마다 **런타임**에 정해진다: 뷰어가 접속 직후 `ScapHello`
(`scap_stream.h` — magic + codec spec 문자열)를 보내고, 서버는 그 spec으로
`ScapEnc_Create(codec)`를 접속별로 생성한다(잘못된 spec은 접속 거부).
scapdec은 설정이 없다 — 패킷 매직('SCP2'/'SCA1')으로 자동 판별한다.
`common/config.h`는 기본값만 가진다(`SCAP_CODEC_DEFAULT "zstd:6"` 등).

Spec 문법:

- `zstd[:level][:dither]` — 8bpp 팔레트 + 스트리밍 zstd. level 1~22
  (생략 시 `ZSTD_LEVEL`=6), `dither`를 붙이면 256컬러 양자화에 4x4 Bayer
  ordered dithering 적용(그라데이션 밴딩 감소, 패킷은 커짐). 디더는 인코더가
  인덱스를 고르는 방식만 바꾸므로(팔레트 동일) 디코더는 영향 없음.
  옵션 순서 무관. 예: `zstd:6`, `zstd:6:dither`
- `av1[:i420|:i444]` — **AV1**. scapenc가 캡처 프레임 전체를 libaom으로 인코딩
  ...
  `SCAP_AV1_BITRATE_KBPS`(기본 8000) 고정 CBR. `i420` = 4:2:0(CRD/WebRTC와
  동일), `i444` = 4:4:4 풀해상도 크로마(profile 1, 색 경계 더 선명·패킷
  커짐), 생략 시 `SCAP_AV1_I444` 기본값.

뷰어 타이틀바에 현재 codec spec이 표시된다. Windows 바이너리는 두 코덱을
모두 내장하므로 재빌드 없이 스위칭된다. 예외는 Linux 뷰어(viewer_x11):
인라인 디코더가 여전히 빌드 타임(`config.h USE_AV1`/`SCAP_AV1_I444`) 선택이라,
hello에 자기 빌드의 spec(`SCAP_CODEC_SPEC`)을 보낸다. AV1 빌드에는
`libdav1d-dev`가 필요하다(`connect.sh`가 없으면 안내 후 중단;
`./viewer --selftest`는 내장 AV1 키프레임을 dav1d로 디코딩해 검증).
...
- `test.exe -selftest` — 비대화형 파이프라인 검증(캡처 1프레임 → 디코딩 → 크기/영역
  일치 확인)을 codec spec(`zstd:6`, `zstd:6:dither`, `av1:i420`, `av1:i444`) 전부에 대해 수행하고,
  잘못된 spec 거부도 확인. exit code 0 = 통과. 대화형 실행은 spec을 인자로 받는다
  (`test.exe av1:i444`, 기본 `SCAP_CODEC_DEFAULT`).
...
```
streamserver.exe                                  # TCP 44300에서 대기
viewer.exe [--addr=host[:port]] [--codec=spec]    # 접속 → 화면 표시
```

- `--addr` 기본값은 `10.1.110.27:44300`, `--codec` 기본값은 `zstd:6`.
  예: `viewer.exe --addr=127.0.0.1:44300 --codec=av1:i444`.
- 뷰어가 hello로 보낸 codec spec으로 서버가 접속별 인코더를 만든다 —
  재빌드 없이 접속마다 코덱 변경 가능.
...
```
Hello  = u32 magic 'SHLO' + char[28] codec spec   // 뷰어가 접속 직후 1회 전송

Packet = ScapFrameHdr(16B) + zstd blob                                    // zstd 코덱, magic 'SCP2'
...
Packet = ScapFrameHdr(16B) + AV1 temporal unit                            // av1 코덱, magic 'SCA1'
  rectCount = moveCount = 0, rawSize = OBU 바이트 길이. 항상 전체 프레임.
  스트림 순서로 한 디코더에 공급; 새 디코더는 키프레임에서만 합류
  (접속별로 새 인코더가 생성되므로 첫 패킷이 항상 키프레임).
```

zstd blob은 인코더의 **연속 zstd frame을 패킷마다 flush한 조각**이다: 패킷은 독립
디코딩이 불가능하고 스트림 순서대로 하나의 DStream에 공급해야 한다(서버가 접속별로
새 인코더를 만들므로 첫 패킷이 항상 frame boundary + 전체 프레임; 세션 도중 리셋은
`ScapEnc_RequestFullFrame()`). 상세는 `common/scap_packet.h` 주석 참조.
